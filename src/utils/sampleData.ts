...
export const INITIAL_TOPICS: Topic[] = [
  ...
  // =========================================================================
  // LEVEL 3 SUB-SUB-TOPICS (Comprehensive Conceptual Knowledge Nodes)
  // =========================================================================
  makeTopic('dm-sub-balls-1', 'subj-dm', 'dm-3', 'Distinguishable Balls in Distinguishable Bins', 'Distributing n distinct items into k distinct boxes (Total: k^n, Injective: P(k,n), Surjective: k! * S(n,k)).', 1, false),
  makeTopic('dm-sub-balls-2', 'subj-dm', 'dm-3', 'Stars and Bars Theorem (Indistinguishable Balls)', 'Distributing n identical items into k distinct boxes C(n+k-1, k-1) and positive constraints C(n-1, k-1).', 2, false),
  makeTopic('dm-sub-balls-3', 'subj-dm', 'dm-3', 'Stirling Numbers of the Second Kind S(n,k)', 'Partitioning n distinct items into k non-empty identical subsets: S(n,k) = k*S(n-1,k) + S(n-1,k-1).', 3, false),
  makeTopic('dm-sub-balls-4', 'subj-dm', 'dm-3', 'Integer Partitions P(n,k)', 'Partitioning n identical items into k identical non-empty parts and generating functions.', 4, false),
  makeTopic('dm-sub-comb-1', 'subj-dm', 'dm-1', 'Derangements & Subfactorial Formula', 'Derangement formula D(n) = n! * Sum((-1)^i / i!) and recursive relations D(n) = (n-1)(D(n-1)+D(n-2)).', 1, false),
  makeTopic('dm-sub-comb-2', 'subj-dm', 'dm-1', 'Inclusion-Exclusion Principle Applications', 'Symmetric difference, union of n sets, and counting surjective onto mappings.', 2, false),
  makeTopic('dm-sub-pigeon-1', 'subj-dm', 'dm-8', 'Basic & Generalized Pigeonhole Bounds', 'Ceiling bounds ceil(n/k), sequence monotonicity (Erdos-Szekeres theorem), and divisibility proofs.', 1, false),
  makeTopic('dm-sub-genfunc-1', 'subj-dm', 'dm-5', 'Ordinary Generating Functions & Convolutions', 'Closed forms for geometric series 1/(1-x)^k, coefficient extraction, and polynomial multiplications.', 1, false),
  makeTopic('dm-sub-planar-1', 'subj-dm', 'dm-11', 'Euler Characteristic Formula', 'Planar graph formula V - E + R = 1 + k (connected components) and face degree sum 2E = Sum(deg(f)).', 1, false),
  makeTopic('dm-sub-planar-2', 'subj-dm', 'dm-11', 'Planarity Bounds & Kuratowski Theorem', 'Planar edge upper bounds E <= 3V - 6 and bipartite planar E <= 2V - 4; non-planarity of K5 and K3,3.', 2, false),
  makeTopic('dm-sub-color-1', 'subj-dm', 'dm-12', 'Chromatic Number & Four Color Theorem', 'Vertex coloring bounds chi(G) <= Delta(G) + 1 (Brooks theorem), planar coloring chi(G) <= 4.', 1, false),
  makeTopic('dm-sub-prop-1', 'subj-dm', 'dm-17', 'Logical Equivalences & Normal Forms', 'De Morgan laws, contrapositive, implication elimination (p -> q equiv ~p v q), and CNF/DNF conversions.', 1, false),
  makeTopic('dm-sub-pred-1', 'subj-dm', 'dm-18', 'First-Order Logic Quantifiers', 'Universal (forall) and existential (exists) scope, quantifier negation rules, and domain interpretations.', 1, false),
  makeTopic('dm-sub-rel-1', 'subj-dm', 'dm-20', 'Relation Properties & Equivalence Classes', 'Reflexive, symmetric, transitive, and antisymmetric properties; counting relation types on n elements.', 1, false),
  makeTopic('dm-sub-poset-1', 'subj-dm', 'dm-25', 'Hasse Diagrams & Lattice Properties', 'Poset partial ordering, maximal/minimal vs greatest/least elements, and distributive/complemented lattices.', 1, false),
  makeTopic('dm-sub-grp-1', 'subj-dm', 'dm-21', 'Group Axioms & Lagrange Theorem', 'Closure, associativity, identity, and inverse; subgroup orders divide group order |H| divides |G|.', 1, false),
  makeTopic('coa-sub-mem-1', 'subj-coa', 'coa-1', 'Direct Mapped Cache Organization', 'Address breakdown: Tag, Line (Index), and Block (Byte) Offset; mapping block i to i mod N.', 1, false),
  makeTopic('coa-sub-mem-2', 'subj-coa', 'coa-1', 'k-Way Set-Associative Cache', 'Address breakdown: Tag, Set Index (log2(Sets)), and Byte Offset; parallel comparator hardware.', 2, false),
  makeTopic('coa-sub-mem-3', 'subj-coa', 'coa-1', 'Multi-Level Cache AMAT Hierarchy', 'Average Memory Access Time: AMAT = L1_Hit_Time + L1_Miss_Rate * (L2_Hit_Time + L2_Miss_Rate * Miss_Penalty).', 3, false),
  makeTopic('coa-sub-mem-4', 'subj-coa', 'coa-1', 'Cache Replacement & Write Policies', 'LRU, FIFO, Optimal replacement; Write-Through vs Write-Back (dirty bit) and Write-Allocate vs No-Allocate.', 4, false),
  makeTopic('coa-sub-pipe-1', 'subj-coa', 'coa-2', 'Data Hazards & Operand Forwarding', 'Read-After-Write (RAW), Write-After-Read (WAR), Write-After-Write (WAW) dependencies and EX-to-EX forwarding.', 1, false),
  makeTopic('coa-sub-pipe-2', 'subj-coa', 'coa-2', 'Control Hazards & Branch Prediction', 'Branch penalty cycles, delayed branching, branch target buffer (BTB), and static/dynamic prediction.', 2, false),
  makeTopic('coa-sub-pipe-3', 'subj-coa', 'coa-2', 'Pipeline Speedup & CPI Analysis', 'Speedup S = (k + n - 1) / (k + n - 1 + stalls) and ideal CPI = 1 versus stall cycle overhead.', 3, false),
  makeTopic('coa-sub-addr-1', 'subj-coa', 'coa-4', 'Register & Immediate Addressing', 'Zero-memory operands, small constant encoding, and register-to-register ALUs.', 1, false),
  makeTopic('coa-sub-addr-2', 'subj-coa', 'coa-4', 'Displacement & PC-Relative Addressing', 'Base-register, indexed, and PC-relative offset calculations for branch target addresses.', 2, false),
  makeTopic('coa-sub-ieee-1', 'subj-coa', 'coa-sub-fp', 'IEEE 754 Single Precision 32-bit Format', '1-bit Sign, 8-bit Exponent (Bias 127), 23-bit Fraction/Mantissa: (-1)^S * 1.F * 2^(E-127).', 1, false),
  makeTopic('coa-sub-ieee-2', 'subj-coa', 'coa-sub-fp', 'Special Values & Denormalized Numbers', 'Exponent 0 (Denorm / Zero), Exponent 255 (Infinity if F=0, NaN if F!=0).', 2, false),
  makeTopic('cn-sub-ip-1', 'subj-cn', 'cn-1', 'IPv4 CIDR Subnetting & Masking', 'Subnet ID, Directed Broadcast Address, Host range calculations and /prefix notation.', 1, false),
  makeTopic('cn-sub-ip-2', 'subj-cn', 'cn-1', 'Variable Length Subnet Masking (VLSM)', 'Hierarchical variable subnet allocation minimizing IP address waste.', 2, false),
  makeTopic('cn-sub-rout-1', 'subj-cn', 'cn-2', 'Distance Vector Routing (Bellman-Ford)', 'Periodic routing updates, count-to-infinity problem, split horizon, and poison reverse.', 1, false),
  makeTopic('cn-sub-rout-2', 'subj-cn', 'cn-2', 'Link State Routing (OSPF & Dijkstra)', 'Link State Advertisements (LSA), reliable flooding, and shortest path tree computation.', 2, false),
  makeTopic('cn-sub-rout-3', 'subj-cn', 'cn-2', 'Longest Prefix Match Forwarding', 'Routing table lookups using binary trie matching on destination IP prefixes.', 3, false),
  makeTopic('cn-sub-sw-1', 'subj-cn', 'cn-3', 'Stop and Wait ARQ Analysis', 'Efficiency eta = 1 / (1 + 2a) where a = T_prop / T_trans and throughput calculations.', 1, false),
  makeTopic('cn-sub-sw-2', 'subj-cn', 'cn-3', 'Go-Back-N vs Selective Repeat Window Bounds', 'GBN sender window <= 2^k - 1, receiver = 1; Selective Repeat sender and receiver <= 2^(k-1).', 2, false),
  makeTopic('cn-sub-tcp-1', 'subj-cn', 'cn-4', 'TCP 3-Way Handshake & Connection Teardown', 'SYN, SYN-ACK, ACK sequence number synchronization and FIN/ACK 4-way close.', 1, false),
  makeTopic('cn-sub-tcp-2', 'subj-cn', 'cn-4', 'TCP Congestion Control Dynamics', 'Slow Start (exponential cwnd doubling), Congestion Avoidance (additive increase), Fast Retransmit (3 dup ACKs).', 2, false),
  makeTopic('db-sub-acid-1', 'subj-db', 'db-1', 'ACID Properties & Transaction States', 'Atomicity (Undo log), Consistency, Isolation (Concurrency control), Durability (Redo log).', 1, false),
  makeTopic('db-sub-ser-1', 'subj-db', 'db-2', 'Conflict Serializability & Precedence Graphs', 'Conflict operations (RW, WR, WW on same item), cycle detection in precedence graph for serializability.', 1, false),
  makeTopic('db-sub-ser-2', 'subj-db', 'db-2', 'Recoverable & Cascadeless Schedules', 'Recoverable (commit after write transaction commits), Cascadeless (read only committed items).', 2, false),
  makeTopic('db-sub-fd-1', 'subj-db', 'db-3', 'Attribute Closure & Candidate Key Finding', 'Computing X+ using Armstrong axioms and finding all minimal candidate keys.', 1, false),
  makeTopic('db-sub-fd-2', 'subj-db', 'db-3', 'Canonical Cover of Functional Dependencies', 'Minimal cover: removing extraneous attributes on LHS/RHS and redundant FDs.', 2, false),
  makeTopic('db-sub-norm-1', 'subj-db', 'db-6', '3NF vs BCNF Decomposition Conditions', '3NF allows prime attribute on RHS; BCNF strictly requires LHS to be superkey; losslessness and dependency preservation.', 1, false),
  makeTopic('db-sub-btree-1', 'subj-db', 'db-8', 'B+ Tree Index Calculations & Order Constraints', 'Block size B, Record pointer Rp, Block pointer Bp, Key K: order p satisfies p*Bp + (p-1)*K <= B.', 1, false),
  makeTopic('dl-sub-kmap-1', 'subj-dld', 'dl-4', 'Prime & Essential Prime Implicants', 'Identifying largest minterm groupings, essential prime implicants covering unique minterms, and dont cares.', 1, false),
  makeTopic('dl-sub-mux-1', 'subj-dld', 'dl-2', 'Multiplexer Function Synthesis', 'Implementing n-variable boolean functions using 2^(n-1) : 1 MUX and Shannon expansion theorem.', 1, false),
  makeTopic('dl-sub-adder-1', 'subj-dld', 'dl-2', 'Carry Lookahead vs Ripple Carry Adders', 'Generate G_i = A_i*B_i and Propagate P_i = A_i ^ B_i signals, eliminating ripple propagation delays.', 2, false),
  makeTopic('dl-sub-ff-1', 'subj-dld', 'dl-3', 'Flip-Flop Characteristic & Excitation Tables', 'JK Flip-Flop Q_next = J*~Q + ~K*Q, T Flip-Flop Q_next = T ^ Q, D Flip-Flop Q_next = D.', 1, false),
  makeTopic('dl-sub-timing-1', 'subj-dld', 'dl-3', 'Setup & Hold Time Clock Constraints', 'Clock period T_clk >= t_cq + t_comb + t_setup; hold time constraint t_cq + t_comb >= t_hold.', 2, false),
  makeTopic('dl-sub-cnt-1', 'subj-dld', 'dl-3', 'Synchronous & Modulo-N Counter Design', 'State transition diagrams, unused state lockup prevention, and Johnson/Ring counter sequences.', 3, false),
  makeTopic('os-sub-page-1', 'subj-os', 'os-2', 'Multi-Level Paging Address Translation', 'Outer page table, inner page table indices, offset bits, and physical address frame concatenation.', 1, false),
  makeTopic('os-sub-tlb-1', 'subj-os', 'os-2', 'Effective Memory Access Time with TLB', 'EAT = h * (t_tlb + t_m) + (1 - h) * (t_tlb + (levels + 1) * t_m) calculations.', 2, false),
  makeTopic('os-sub-sched-1', 'subj-os', 'os-3', 'Shortest Remaining Time First (SRTF) Scheduling', 'Preemptive shortest CPU burst tracking, Gantt chart scheduling, and waiting time formulas.', 1, false),
  makeTopic('os-sub-sched-2', 'subj-os', 'os-3', 'Round Robin Quantum Optimization', 'Context switch penalty, response time vs turnaround trade-offs with large/small quantum sizes.', 2, false),
  makeTopic('os-sub-sync-1', 'subj-os', 'os-4', 'Classical Synchronization Semaphores', 'Producer-Consumer bounded buffer, Readers-Writers priority, and Dining Philosophers deadlock prevention.', 1, false),
  makeTopic('os-sub-bank-1', 'subj-os', 'os-5', 'Banker Algorithm Safety Check', 'Available, Allocation, Max, and Need matrices; safe sequence validation and resource request algorithm.', 1, false),
  makeTopic('alg-sub-master-1', 'subj-algo', 'alg-2', 'Master Theorem Cases & Conditions', 'Standard cases for T(n) = aT(n/b) + Theta(n^k log^p n) and non-polynomial gaps.', 1, false),
  makeTopic('alg-sub-asymp-1', 'subj-algo', 'alg-6', 'Formal Limit Definitions of Asymptotic Notations', 'Limit n->inf f(n)/g(n) = 0 (Little-o), c > 0 (Theta), inf (Little-omega) comparison proofs.', 1, false),
  makeTopic('alg-sub-qsort-1', 'subj-algo', 'alg-8', 'Hoare vs Lomuto Partitioning Schemes', 'Pointer mechanics, swap counts, and recursion tree depth under sorted/reverse-sorted inputs.', 1, false),
  makeTopic('alg-sub-mst-1', 'subj-algo', 'alg-3', 'Kruskal Algorithm with Disjoint Set Union', 'Union by rank and path compression O(E * alpha(V)) inverse Ackermann time bounds.', 1, false),
  makeTopic('alg-sub-dijk-1', 'subj-algo', 'alg-sub-dijkstra', 'Dijkstra Min-Heap Implementation & Bounds', 'Vertex relaxation step, O((V + E) log V) complexity, and negative edge limitations.', 1, false),
  makeTopic('alg-sub-dp-1', 'subj-algo', 'alg-10', '0/1 Knapsack & Longest Common Subsequence', '2D DP state transitions, space optimization to 1D arrays, and backtracking optimal solutions.', 1, false),
  makeTopic('cd-sub-lr-1', 'subj-cd', 'cd-4', 'LR(0) vs SLR(1) Parsing Table Construction', 'Item set closures, goto transitions, shift-reduce conflicts, and Follow set reduction placement.', 1, false),
  makeTopic('cd-sub-lr-2', 'subj-cd', 'cd-4', 'LALR(1) State Merging & Conflicts', 'Merging CLR(1) states with identical LR(0) cores and handling resulting reduce-reduce conflicts.', 2, false),
  makeTopic('cd-sub-sdt-1', 'subj-cd', 'cd-5', 'S-Attributed vs L-Attributed SDD Evaluation', 'Synthesized bottom-up evaluation on parse tree vs inherited left-to-right top-down attribute flow.', 1, false),
  makeTopic('cd-sub-opt-1', 'subj-cd', 'cd-sub-opt', 'Basic Block Optimization & DAGs', 'Leader instruction identification, local common subexpressions, and dead code elimination in DAGs.', 1, false),
  makeTopic('ds-sub-bst-1', 'subj-ds', 'ds-2', 'BST Deletion & Inorder Successor', 'Deleting node with 2 children using inorder successor / predecessor and tree restructuring.', 1, false),
  makeTopic('ds-sub-avl-1', 'subj-ds', 'ds-10', 'AVL Tree LL, RR, LR, RL Rotations', 'Single and double rotations to restore balance factor in {-1, 0, 1} with O(1) pointer updates.', 1, false),
  makeTopic('ds-sub-heap-1', 'subj-ds', 'ds-3', 'Build-Heap Linear Time Complexity Proof', 'Sum of heights proof Sum(i * 2^(h-i)) = O(n) for linear heap construction.', 1, false),
  makeTopic('ds-sub-stack-1', 'subj-ds', 'ds-5', 'Infix to Postfix Conversion using Stack', 'Operator precedence, left/right associativity stack rules, and expression evaluation.', 1, false),
  makeTopic('pr-sub-ptr-1', 'subj-prog', 'pr-3', 'Pointer Arithmetic & Multi-Dimensional Arrays', 'Scale factor multiplication address calculations *(a + i) + j and array-to-pointer decay.', 1, false),
  makeTopic('pr-sub-ptr-2', 'subj-prog', 'pr-3', 'Function Pointers & Dynamic Allocation', 'Declaring void (*fp)(int), callback functions, malloc, calloc, realloc, and memory leaks.', 2, false),
  makeTopic('pr-sub-rec-1', 'subj-prog', 'pr-2', 'Recursive Activation Stack Trace', 'Activation records, local frame pointers, static vs dynamic scoping, and recursion unwind returns.', 1, false),
  makeTopic('toc-sub-dfa-1', 'subj-toc', 'toc-1', 'Subset Construction (NFA to DFA)', 'Power set state construction, dead states, and determining minimal DFA states.', 1, false),
  makeTopic('toc-sub-pump-1', 'subj-toc', 'toc-6', 'Pumping Lemma for Regular Languages', 'Pumping length p, decomposition w = xyz with |xy| <= p, |y| >= 1, proving languages non-regular.', 1, false),
  makeTopic('toc-sub-pda-1', 'subj-toc', 'toc-9', 'DPDA vs NPDA Languages', 'Deterministic CFLs (proper subset of CFLs) and acceptance by empty stack vs final state.', 1, false),
  makeTopic('toc-sub-dec-1', 'subj-toc', 'toc-5', 'Rice Theorem on RE Languages', 'Non-trivial semantic properties of recursively enumerable languages are undecidable.', 1, false),
  makeTopic('em-sub-eigen-1', 'subj-em', 'em-1', 'Eigenvalues, Eigenvectors & Cayley-Hamilton', 'Trace = sum of eigenvalues, Determinant = product of eigenvalues; computing matrix powers A^k.', 1, false),
  makeTopic('em-sub-prob-1', 'subj-em', 'em-11', 'Bayes Theorem & Total Probability', 'P(B_i | A) = P(A | B_i)*P(B_i) / Sum(P(A | B_j)*P(B_j)) and conditional expectation.', 1, false),
  makeTopic('em-sub-calc-1', 'subj-em', 'em-22', 'Maxima, Minima & Saddle Points (Hessian Matrix)', 'Second derivative test for 2 variables: D = f_xx * f_yy - (f_xy)^2 sign analysis.', 1, false),
  makeTopic('ga-sub-tw-1', 'subj-ga', 'ga-sub-time-work', 'Pipes & Cisterns Inlets/Outlets', 'Positive filling rates vs negative emptying rates and alternating work cycles.', 1, false),
  makeTopic('ga-sub-spd-1', 'subj-ga', 'ga-sub-spd', 'Relative Speed, Trains & Boats/Streams', 'Opposite vs same direction relative velocity, upstream (u - v) and downstream (u + v) formulas.', 1, false),
  makeTopic('ga-sub-geom-1', 'subj-ga', 'ga-sub-geom', 'Circle Tangents & 3D Mensuration', 'Secant-tangent theorem, similar triangle ratios, and cone/cylinder volume optimizations.', 1, false),
];

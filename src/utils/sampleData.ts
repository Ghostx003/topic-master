...
export const INITIAL_TOPICS: Topic[] = [
  // ================= 1. COMPUTER NETWORKS =================
  // Heavy Hitters: IPV4 Addressing, Error Control, IPV4 Header, TCP & UDP, MAC Layer, Routing Protocols
  makeTopic('top-cn-1', 'subj-cn', null, 'IPV4 Addressing', 1, { star: true, confidence: 'High', practice: true }),
  makeTopic('sub-cn-1-1', 'subj-cn', 'top-cn-1', 'Classful Addressing & Net ID / Host ID', 1, { done: true }),
  makeTopic('sub-cn-1-2', 'subj-cn', 'top-cn-1', 'CIDR & Classless Subnetting', 2, { star: true, practice: true }),
  makeTopic('sub-cn-1-3', 'subj-cn', 'top-cn-1', 'VLSM & Variable Length Subnet Masking', 3, { star: true }),
  ...
  makeTopic('top-cn-2', 'subj-cn', null, 'Error Control & Flow Control', 2, { star: true, practice: true }),
  makeTopic('sub-cn-2-1', 'subj-cn', 'top-cn-2', 'Stop and Wait ARQ & Efficiency', 1, { star: true }),
  makeTopic('sub-cn-2-2', 'subj-cn', 'top-cn-2', 'Go-Back-N (GBN) Protocol & Window Sizing', 2, { star: true }),
  makeTopic('sub-cn-2-3', 'subj-cn', 'top-cn-2', 'Selective Repeat (SR) ARQ', 3, { star: true }),
  makeTopic('sub-cn-2-4', 'subj-cn', 'top-cn-2', 'CRC (Cyclic Redundancy Check) Generator Polynomial', 4, { star: true }),
  makeTopic('sub-cn-2-5', 'subj-cn', 'top-cn-2', 'Hamming Code & Error Correction', 5),

  makeTopic('top-cn-3', 'subj-cn', null, 'IPV4 Header & Fragmentation', 3, { star: true }),
  makeTopic('sub-cn-3-1', 'subj-cn', 'top-cn-3', 'IPv4 Header Fields & Checksum', 1),
  makeTopic('sub-cn-3-2', 'subj-cn', 'top-cn-3', 'MTU and Fragmentation Offset Calculation', 2, { star: true, practice: true }),
  makeTopic('sub-cn-3-3', 'subj-cn', 'top-cn-3', 'Identification, Flags (DF, MF)', 3),

  makeTopic('top-cn-4', 'subj-cn', null, 'TCP & UDP', 4, { star: true, confidence: 'Medium', practice: true }),
  makeTopic('sub-cn-4-1', 'subj-cn', 'top-cn-4', 'TCP Header Format & Flags (SYN, ACK, FIN, RST)', 1, { star: true }),
  makeTopic('sub-cn-4-2', 'subj-cn', 'top-cn-4', 'TCP 3-Way Handshake & Connection Teardown', 2, { star: true }),
  makeTopic('sub-cn-4-3', 'subj-cn', 'top-cn-4', 'Flow Control & Sliding Window in TCP', 3),
  makeTopic('sub-cn-4-4', 'subj-cn', 'top-cn-4', 'Congestion Control: Slow Start, AIMD, Fast Retransmit', 4, { star: true, practice: true }),
  makeTopic('sub-cn-4-5', 'subj-cn', 'top-cn-4', 'UDP Header & Stateless Transport', 5),

  makeTopic('top-cn-5', 'subj-cn', null, 'Medium Access Control', 5, { star: true }),
  makeTopic('sub-cn-5-1', 'subj-cn', 'top-cn-5', 'Pure ALOHA & Slotted ALOHA Efficiency', 1),
  makeTopic('sub-cn-5-2', 'subj-cn', 'top-cn-5', 'CSMA/CD & Minimum Frame Size Condition', 2, { star: true, practice: true }),
  ...
  makeTopic('top-cn-6', 'subj-cn', null, 'Routing Protocols & Switching', 6, { star: true, practice: true }),
  makeTopic('sub-cn-6-1', 'subj-cn', 'top-cn-6', 'Distance Vector Routing & Count-to-Infinity Problem', 1, { star: true }),
  makeTopic('sub-cn-6-2', 'subj-cn', 'top-cn-6', 'Link State Routing & Dijkstra Shortest Path', 2, { star: true }),
  ...
  // ================= 2. OPERATING SYSTEMS =================
  // Heavy Hitters: CPU Scheduling, Process Synchronization, DeadLock, Memory Management
  ...
  makeTopic('top-os-2', 'subj-os', null, 'Process Management', 2),
  ...
  makeTopic('top-os-3', 'subj-os', null, 'CPU Scheduling', 3, { star: true, confidence: 'High', practice: true }),
  makeTopic('sub-os-3-1', 'subj-os', 'top-os-3', 'FCFS (First-Come, First-Served) & Convoy Effect', 1, { done: true }),
  makeTopic('sub-os-3-2', 'subj-os', 'top-os-3', 'SJF & SRTF (Shortest Job / Remaining Time First)', 2, { star: true, done: true }),
  makeTopic('sub-os-3-3', 'subj-os', 'top-os-3', 'Round Robin Scheduling & Time Quantum Tuning', 3, { star: true, done: true }),
  ...
  makeTopic('sub-os-4-1', 'subj-os', 'top-os-4', 'Critical Section Problem & Hardware Instructions (TSL)', 1, { star: true }),
  makeTopic('sub-os-4-2', 'subj-os', 'top-os-4', 'Peterson’s Algorithm for 2 Processes', 2, { star: true }),
  makeTopic('sub-os-4-3', 'subj-os', 'top-os-4', 'Counting and Binary Semaphores (Wait & Signal)', 3, { star: true, practice: true }),
  makeTopic('sub-os-4-4', 'subj-os', 'top-os-4', 'Producer-Consumer / Bounded Buffer Problem', 4, { star: true }),
  makeTopic('sub-os-4-5', 'subj-os', 'top-os-4', 'Readers-Writers Problem', 5, { star: true }),
  makeTopic('sub-os-4-6', 'subj-os', 'top-os-4', 'Dining Philosophers Problem', 6),

  makeTopic('top-os-5', 'subj-os', null, 'DeadLock', 5, { star: true, practice: true }),
  makeTopic('sub-os-5-1', 'subj-os', 'top-os-5', '4 Necessary Coffman Conditions', 1, { done: true }),
  makeTopic('sub-os-5-2', 'subj-os', 'top-os-5', 'Resource Allocation Graph (RAG) & Cycle Detection', 2, { star: true }),
  makeTopic('sub-os-5-3', 'subj-os', 'top-os-5', 'Deadlock Prevention vs Avoidance', 3),
  makeTopic('sub-os-5-4', 'subj-os', 'top-os-5', 'Banker’s Algorithm (Safety & Resource Request)', 4, { star: true, practice: true }),
  ...
  makeTopic('sub-os-6-2', 'subj-os', 'top-os-6', 'Paging & Single-Level Page Tables', 2, { star: true }),
  makeTopic('sub-os-6-3', 'subj-os', 'top-os-6', 'Multi-Level Paging & Inverted Page Tables', 3, { star: true }),
  makeTopic('sub-os-6-4', 'subj-os', 'top-os-6', 'Translation Lookaside Buffer (TLB) & Effective Memory Access Time', 4, { star: true, practice: true }),
  makeTopic('sub-os-6-5', 'subj-os', 'top-os-6', 'Page Replacement: FIFO, Optimal, LRU, Second-Chance', 5, { star: true, practice: true }),
  ...
  // ================= 3. C-PROGRAMMING =================
  // Heavy Hitters: Data Types & Operators, Functions & Storage Classes, Arrays & Pointers
  makeTopic('top-cp-1', 'subj-prog', null, 'Data Types & Operators', 1, { star: true, done: true }),
  makeTopic('sub-cp-1-1', 'subj-prog', 'top-cp-1', 'Data Types, Range & Type Promotion Rules', 1, { done: true }),
  makeTopic('sub-cp-1-2', 'subj-prog', 'top-cp-1', 'Operator Precedence & Associativity Matrix', 2, { star: true, done: true }),
  makeTopic('sub-cp-1-3', 'subj-prog', 'top-cp-1', 'Bitwise Operators (&, |, ^, ~, <<, >>)', 3, { star: true, practice: true }),
  ...
  makeTopic('sub-cp-3-2', 'subj-prog', 'top-cp-3', 'Recursive Functions & Stack Frame Analysis', 2, { star: true, practice: true }),
  ...
  makeTopic('sub-cp-4-1', 'subj-prog', 'top-cp-4', '1D and 2D Arrays (Row-Major vs Column-Major Mapping)', 1, { star: true }),
  makeTopic('sub-cp-4-2', 'subj-prog', 'top-cp-4', 'Pointer Arithmetic & Dereferencing Expressions', 2, { star: true }),
  makeTopic('sub-cp-4-3', 'subj-prog', 'top-cp-4', 'Array of Pointers vs Pointer to an Array', 3, { star: true }),
  ...
  // ================= 4. DATA STRUCTURES =================
  // Heavy Hitters: Linked List, Stack & Queues, Trees, Graphs, Hashing
  ...
  makeTopic('top-ds-2', 'subj-ds', null, 'Linked List', 2, { star: true }),
  ...
  makeTopic('top-ds-3', 'subj-ds', null, 'Stack & Queues', 3, { star: true, practice: true }),
  makeTopic('sub-ds-3-1', 'subj-ds', 'top-ds-3', 'Stack Implementation & Infix to Postfix Conversion', 1, { star: true }),
  makeTopic('sub-ds-3-2', 'subj-ds', 'top-ds-3', 'Postfix & Prefix Expression Evaluation', 2, { star: true, practice: true }),
  ...
  makeTopic('sub-ds-4-1', 'subj-ds', 'top-ds-4', 'Binary Tree Properties & Tree Traversal (In, Pre, Post, Level)', 1, { star: true }),
  makeTopic('sub-ds-4-2', 'subj-ds', 'top-ds-4', 'Reconstructing Tree from Traversals', 2, { star: true }),
  makeTopic('sub-ds-4-3', 'subj-ds', 'top-ds-4', 'Binary Search Tree (BST) Operations & Inorder Successor', 3, { star: true }),
  makeTopic('sub-ds-4-4', 'subj-ds', 'top-ds-4', 'AVL Trees & Rotations (LL, RR, LR, RL)', 4, { star: true, practice: true }),
  makeTopic('sub-ds-4-5', 'subj-ds', 'top-ds-4', 'Binary Heaps (Min Heap & Max Heap Construction)', 5, { star: true }),
  ...
  makeTopic('sub-ds-5-2', 'subj-ds', 'top-ds-5', 'Breadth-First Search (BFS) & Shortest Path in Unweighted Graph', 2, { star: true }),
  makeTopic('sub-ds-5-3', 'subj-ds', 'top-ds-5', 'Depth-First Search (DFS) & Classification of Edges', 3, { star: true }),

  makeTopic('top-ds-6', 'subj-ds', null, 'Hashing', 6, { star: true, practice: true }),
  makeTopic('sub-ds-6-1', 'subj-ds', 'top-ds-6', 'Hash Functions & Collision Resolution using Chaining', 1),
  makeTopic('sub-ds-6-2', 'subj-ds', 'top-ds-6', 'Open Addressing: Linear Probing, Quadratic Probing, Double Hashing', 2, { star: true, practice: true }),

  // ================= 5. DIGITAL LOGIC =================
  // Heavy Hitters: Logic Gates & Minimization, Combinational Circuit, Sequential Circuit, Number System
  makeTopic('top-dl-1', 'subj-dl', null, 'Logic Gates & Minimization', 1, { star: true, done: true }),
  ...
  makeTopic('sub-dl-1-3', 'subj-dl', 'top-dl-1', 'Karnaugh Maps (K-Maps) 2, 3, 4 Variables & Don’t Cares', 3, { star: true, practice: true }),
  makeTopic('sub-dl-1-4', 'subj-dl', 'top-dl-1', 'Prime Implicants & Essential Prime Implicants', 4, { star: true }),

  makeTopic('top-dl-2', 'subj-dl', null, 'Combinational Circuit', 2, { star: true }),
  makeTopic('sub-dl-2-1', 'subj-dl', 'top-dl-2', 'Half Adder, Full Adder, Ripple Carry & Carry Lookahead Adder', 1, { star: true }),
  ...
  makeTopic('sub-dl-3-2', 'subj-dl', 'top-dl-3', 'Master-Slave Flip-Flop & Race-Around Condition', 2, { star: true }),
  makeTopic('sub-dl-3-3', 'subj-dl', 'top-dl-3', 'Synchronous & Asynchronous Up/Down Counters', 3, { star: true, practice: true }),
  makeTopic('sub-dl-3-4', 'subj-dl', 'top-dl-3', 'Ring Counter & Johnson (Twisted Ring) Counter', 4),

  makeTopic('top-dl-4', 'subj-dl', null, 'Number System', 4, { star: true, done: true }),
  makeTopic('sub-dl-4-1', 'subj-dl', 'top-dl-4', 'Base Conversions (Binary, Octal, Decimal, Hexadecimal)', 1, { done: true }),
  makeTopic('sub-dl-4-2', 'subj-dl', 'top-dl-4', '1’s Complement & 2’s Complement Arithmetic & Overflow', 2, { star: true, done: true }),

  // ================= 6. THEORY OF COMPUTATION =================
  // Heavy Hitters: Finite Automata, Push Down Automata, Turing Machine, Decidability (All core TOC)
  makeTopic('top-toc-1', 'subj-toc', null, 'Finite Automata', 1, { star: true, confidence: 'High' }),
  makeTopic('sub-toc-1-1', 'subj-toc', 'top-toc-1', 'DFA Construction & NFA to DFA Subset Construction', 1, { star: true, done: true }),
  makeTopic('sub-toc-1-2', 'subj-toc', 'top-toc-1', 'DFA State Minimization using Myhill-Nerode Theorem', 2, { star: true }),
  ...
  makeTopic('sub-toc-2-1', 'subj-toc', 'top-toc-2', 'Context-Free Grammars (CFG) & Ambiguity in CFG', 1, { star: true }),
  makeTopic('sub-toc-2-2', 'subj-toc', 'top-toc-2', 'Deterministic PDA (DPDA) vs Non-Deterministic PDA (NPDA)', 2, { star: true }),
  makeTopic('sub-toc-2-3', 'subj-toc', 'top-toc-2', 'Closure Properties of Context-Free Languages (CFL)', 3, { star: true }),

  makeTopic('top-toc-3', 'subj-toc', null, 'Turing Machine & Recursively Enumerable', 3, { star: true }),
  makeTopic('sub-toc-3-1', 'subj-toc', 'top-toc-3', 'Turing Machine Definitions & Transition Functions', 1),
  makeTopic('sub-toc-3-2', 'subj-toc', 'top-toc-3', 'Recursive (REC) vs Recursively Enumerable (RE) Languages', 2, { star: true }),
  makeTopic('sub-toc-3-3', 'subj-toc', 'top-toc-3', 'Chomsky Hierarchy of Languages', 3, { star: true }),
  ...
  makeTopic('sub-toc-4-2', 'subj-toc', 'top-toc-4', 'Decidability Table for FA, CFG, CSG, and TM', 2, { star: true, practice: true }),
  makeTopic('sub-toc-4-3', 'subj-toc', 'top-toc-4', 'Post Correspondence Problem (PCP)', 3, { star: true }),

  // ================= 7. COMPILER DESIGN =================
  // Heavy Hitters: Lexical & Syntax Analysis, Syntax Directed Translation, Intermediate Code & Optimization
  makeTopic('top-cd-1', 'subj-cd', null, 'Lexical & Syntax Analysis', 1, { star: true, practice: true }),
  makeTopic('sub-cd-1-1', 'subj-cd', 'top-cd-1', 'Lexical Analyzer & Token Specification', 1),
  makeTopic('sub-cd-1-2', 'subj-cd', 'top-cd-1', 'Elimination of Left Recursion & Left Factoring', 2, { star: true }),
  makeTopic('sub-cd-1-3', 'subj-cd', 'top-cd-1', 'Computation of FIRST and FOLLOW Sets', 3, { star: true, practice: true }),
  makeTopic('sub-cd-1-4', 'subj-cd', 'top-cd-1', 'LL(1) Predictive Parsing Table & Conflicts', 4, { star: true }),
  makeTopic('sub-cd-1-5', 'subj-cd', 'top-cd-1', 'Bottom-Up Parsing: LR(0), SLR(1), CLR(1), LALR(1)', 5, { star: true, practice: true }),

  makeTopic('top-cd-2', 'subj-cd', null, 'Syntax Directed Translation', 2, { star: true }),
  makeTopic('sub-cd-2-1', 'subj-cd', 'top-cd-2', 'Synthesized vs Inherited Attributes', 1, { star: true }),
  makeTopic('sub-cd-2-2', 'subj-cd', 'top-cd-2', 'S-Attributed and L-Attributed SDDs', 2, { star: true }),

  makeTopic('top-cd-3', 'subj-cd', null, 'Intermediate Code & Code Optimization', 3, { star: true }),
  makeTopic('sub-cd-3-1', 'subj-cd', 'top-cd-3', 'Three Address Code: Quadruples, Triples, Indirect Triples', 1, { star: true }),
  makeTopic('sub-cd-3-2', 'subj-cd', 'top-cd-3', 'Basic Blocks & Control Flow Graphs (CFG)', 2, { star: true }),
  makeTopic('sub-cd-3-3', 'subj-cd', 'top-cd-3', 'Loop Optimization, Dead Code Elimination & Constant Folding', 3, { star: true }),

  // ================= 8. ALGORITHMS =================
  // Heavy Hitters: Analysis Of Algorithms, Design Strategies & Greedy, Dynamic Programming, Graph Algorithms, Backtracking/NP
  makeTopic('top-al-1', 'subj-algo', null, 'Analysis Of Algorithms', 1, { star: true, done: true }),
  makeTopic('sub-al-1-1', 'subj-algo', 'top-al-1', 'Asymptotic Notations (Big-O, Omega, Theta)', 1, { star: true, done: true }),
  makeTopic('sub-al-1-2', 'subj-algo', 'top-al-1', 'Master Theorem for Divide-and-Conquer Recurrences', 2, { star: true, done: true }),
  ...
  makeTopic('sub-al-2-1', 'subj-algo', 'top-al-2', 'Merge Sort & Quick Sort Partitioning Complexity', 1, { star: true }),
  makeTopic('sub-al-2-2', 'subj-algo', 'top-al-2', 'Fractional Knapsack Problem', 2),
  makeTopic('sub-al-2-3', 'subj-algo', 'top-al-2', 'Huffman Coding & Prefix Codes Optimization', 3, { star: true, practice: true }),
  ...
  makeTopic('sub-al-3-1', 'subj-algo', 'top-al-3', '0/1 Knapsack Problem', 1, { star: true, practice: true }),
  makeTopic('sub-al-3-2', 'subj-algo', 'top-al-3', 'Longest Common Subsequence (LCS)', 2, { star: true }),
  makeTopic('sub-al-3-3', 'subj-algo', 'top-al-3', 'Matrix Chain Multiplication (MCM)', 3, { star: true }),
  makeTopic('sub-al-3-4', 'subj-algo', 'top-al-3', 'Subset Sum Problem & Coin Change Problem', 4, { star: true }),

  makeTopic('top-al-4', 'subj-algo', null, 'Graph Algorithms & Heap Algorithms', 4, { star: true, practice: true }),
  makeTopic('sub-al-4-1', 'subj-algo', 'top-al-4', 'Minimum Spanning Tree: Prim’s & Kruskal’s Algorithms', 1, { star: true }),
  ...
  makeTopic('sub-al-4-4', 'subj-algo', 'top-al-4', 'HeapSort & Priority Queue Operations', 4, { star: true }),

  makeTopic('top-al-5', 'subj-algo', null, 'Backtracking & Branch-Bound', 5, { star: true }),
  ...
  // Heavy Hitters: FD's and Normalisation, Transaction & Concurrency, Query Language (SQL), File Organisation & Indexing
  makeTopic('top-db-1', 'subj-dbms', null, "FD's and Normalisation", 1, { star: true, practice: true }),
  makeTopic('sub-db-1-1', 'subj-dbms', 'top-db-1', 'Functional Dependencies & Armstrong’s Axioms', 1, { star: true }),
  makeTopic('sub-db-1-2', 'subj-dbms', 'top-db-1', 'Attribute Closure & Finding Candidate Keys', 2, { star: true }),
  makeTopic('sub-db-1-3', 'subj-dbms', 'top-db-1', '1NF, 2NF, 3NF, and BCNF Normal Forms', 3, { star: true, practice: true }),
  makeTopic('sub-db-1-4', 'subj-dbms', 'top-db-1', 'Lossless Join & Dependency Preserving Decomposition', 4, { star: true }),

  makeTopic('top-db-2', 'subj-dbms', null, 'Transaction and Concurrency Control', 2, { star: true, practice: true }),
  makeTopic('sub-db-2-1', 'subj-dbms', 'top-db-2', 'ACID Properties & Transaction State Diagram', 1),
  makeTopic('sub-db-2-2', 'subj-dbms', 'top-db-2', 'Conflict Serializability & Precedence Graphs', 2, { star: true, practice: true }),
  ...
  makeTopic('top-db-4', 'subj-dbms', null, 'Query Language (SQL)', 4, { star: true, practice: true }),
  makeTopic('sub-db-4-1', 'subj-dbms', 'top-db-4', 'Relational Algebra Operations (Select, Project, Join, Division)', 1, { star: true }),
  makeTopic('sub-db-4-2', 'subj-dbms', 'top-db-4', 'SQL Queries, Joins (Inner, Left, Right, Full)', 2, { star: true }),
  makeTopic('sub-db-4-3', 'subj-dbms', 'top-db-4', 'GROUP BY, HAVING, and Aggregate Functions', 3, { star: true }),
  makeTopic('sub-db-4-4', 'subj-dbms', 'top-db-4', 'Correlated Subqueries & Nested Queries', 4, { star: true, practice: true }),

  makeTopic('top-db-5', 'subj-dbms', null, 'File Organisation & Indexing', 5, { star: true, practice: true }),
  ...
  // Heavy Hitters: Machine Instruction & Addressing Modes, Floating Point, Instruction Pipelining, Cache Memory
  ...
  makeTopic('sub-co-2-2', 'subj-coa', 'top-co-2', 'Instruction Formats & Opcode Encoding Range', 2, { star: true }),

  makeTopic('top-co-3', 'subj-coa', null, 'Floating Point Representation', 3, { star: true }),
  ...
  makeTopic('sub-co-5-1', 'subj-coa', 'top-co-5', 'Instruction Pipeline Stages & Clock Cycle Time', 1, { star: true }),
  makeTopic('sub-co-5-2', 'subj-coa', 'top-co-5', 'Pipeline Hazards: Structural, Data (RAW, WAR, WAW), Control', 2, { star: true }),
  makeTopic('sub-co-5-3', 'subj-coa', 'top-co-5', 'Operand Forwarding & Branch Penalty Calculation', 3, { star: true }),
  makeTopic('sub-co-5-4', 'subj-coa', 'top-co-5', 'Pipeline Speedup & Efficiency Formulae', 4, { star: true, practice: true }),
  ...
  makeTopic('sub-co-6-2', 'subj-coa', 'top-co-6', 'Cache Hit Ratio & Average Memory Access Time (AMAT)', 2, { star: true, practice: true }),
  ...
  // Heavy Hitters: Graph Theory, Mathematical Logic, Set Theory, Combinatorics (All 4 are high weightage in GATE)
  ...
  makeTopic('sub-dm-1-2', 'subj-dm', 'top-dm-1', 'Eulerian & Hamiltonian Graphs Conditions', 2, { star: true }),
  makeTopic('sub-dm-1-3', 'subj-dm', 'top-dm-1', 'Planar Graphs, Euler’s Formula (V - E + F = 2) & Region Bounds', 3, { star: true }),
  makeTopic('sub-dm-1-4', 'subj-dm', 'top-dm-1', 'Graph Coloring & Chromatic Number', 4, { star: true }),

  makeTopic('top-dm-2', 'subj-dm', null, 'Mathematical Logic', 2, { star: true, done: true }),
  makeTopic('sub-dm-2-1', 'subj-dm', 'top-dm-2', 'Propositional Logic, Truth Tables & Tautologies', 1, { done: true }),
  makeTopic('sub-dm-2-2', 'subj-dm', 'top-dm-2', 'Predicate Logic Quantifiers (Universal & Existential)', 2, { star: true, done: true }),
  makeTopic('sub-dm-2-3', 'subj-dm', 'top-dm-2', 'Rules of Inference & Logical Equivalences', 3, { star: true }),

  makeTopic('top-dm-3', 'subj-dm', null, 'Set Theory', 3, { star: true }),
  makeTopic('sub-dm-3-1', 'subj-dm', 'top-dm-3', 'Relations: Reflexive, Symmetric, Transitive, Equivalence', 1, { star: true }),
  makeTopic('sub-dm-3-2', 'subj-dm', 'top-dm-3', 'Partial Order Relations, Hasse Diagrams & Lattices', 2, { star: true }),
  makeTopic('sub-dm-3-3', 'subj-dm', 'top-dm-3', 'Functions: Injective, Surjective, Bijective, Pigeonhole Principle', 3, { star: true }),

  makeTopic('top-dm-4', 'subj-dm', null, 'Combinatorics', 4, { star: true, practice: true }),
  makeTopic('sub-dm-4-1', 'subj-dm', 'top-dm-4', 'Permutations & Combinations with Repetition Rules', 1, { star: true }),
  makeTopic('sub-dm-4-2', 'subj-dm', 'top-dm-4', 'Principle of Inclusion-Exclusion (PIE)', 2, { star: true }),
  makeTopic('sub-dm-4-3', 'subj-dm', 'top-dm-4', 'Generating Functions & Solving Recurrence Relations', 3, { star: true }),

  // ================= 12. ENGINEERING MATHEMATICS =================
  // Heavy Hitters: Linear Algebra, Calculus, Probability & Statistics
  ...
  makeTopic('sub-em-1-2', 'subj-em', 'top-em-1', 'System of Linear Equations (Unique, Infinite, No Solution)', 2, { star: true, done: true }),
  makeTopic('sub-em-1-3', 'subj-em', 'top-em-1', 'Eigenvalues, Eigenvectors & Cayley-Hamilton Theorem', 3, { star: true }),

  makeTopic('top-em-2', 'subj-em', null, 'Calculus', 2, { star: true }),
  makeTopic('sub-em-2-1', 'subj-em', 'top-em-2', 'Limits, Continuity & Differentiability', 1, { star: true }),
  makeTopic('sub-em-2-2', 'subj-em', 'top-em-2', 'Maxima, Minima & Mean Value Theorems', 2, { star: true }),
  ...
  makeTopic('sub-em-3-3', 'subj-em', 'top-em-3', 'Uniform, Binomial, Poisson, Exponential, Normal Distributions', 3, { star: true, practice: true }),
  makeTopic('sub-em-3-4', 'subj-em', 'top-em-3', 'Mean, Variance, Standard Deviation & Covariance', 4, { star: true }),

  // ================= 13. GENERAL APTITUDE =================
  // Heavy Hitters: Percentages, Profit and Loss, Time and Work, Speed Distance Time, Data Interpretation, Permutations, Venn Diagrams
  ...
  makeTopic('top-ga-2', 'subj-ga', null, 'Percentages', 2, { star: true, done: true }),
  ...
  makeTopic('top-ga-4', 'subj-ga', null, 'Profit and Loss', 4, { star: true }),
  makeTopic('sub-ga-4-1', 'subj-ga', 'top-ga-4', 'Marked Price, Discount & Dishonest Seller', 1, { star: true }),
  ...
  makeTopic('sub-ga-8-1', 'subj-ga', 'top-ga-8', 'Efficiency Ratio & Work-Wages Calculation', 1, { star: true }),
  ...
  makeTopic('sub-ga-10-1', 'subj-ga', 'top-ga-10', 'Average Speed Formula & Relative Speed Calculations', 1, { star: true }),
  ...
  makeTopic('top-ga-13', 'subj-ga', null, 'Data Interpretation', 13, { star: true, practice: true }),
  makeTopic('sub-ga-13-1', 'subj-ga', 'top-ga-13', 'Pie Charts, Bar Graphs & Tabular Interpretation', 1, { star: true }),
  ...
  makeTopic('sub-ga-15-1', 'subj-ga', 'top-ga-15', 'Arrangements of Words with Repeating Letters', 1, { star: true }),
  ...
  makeTopic('top-ga-23', 'subj-ga', null, 'Venn Digrams', 23, { star: true }),
  makeTopic('sub-ga-23-1', 'subj-ga', 'top-ga-23', 'Syllogisms & 3-Set Logic Puzzles', 1, { star: true }),
